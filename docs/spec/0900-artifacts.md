...
`src/graph.rs`, `src/synthesis.rs`, `src/unit_analysis.rs`,
...
- inspect 산출물은 `artifact_manifest.json`, `brief.json`, `brief.md`,
  ...
  `sensitive.json`, `gates.json`, `slices.json`, `review.json`,
  `security.json`, `connection_graph.json`, `analysis_plan.json`,
  `cross_unit_analysis.json`, `synthesis.json`, `followup_plan.json`,
  `report.md`, `report.html`을 포함한다. agent receipt는
...
- 쓰기 순서 고정: source → inventory → coverage → evidence → findings →
  dependencies → sectors → sensitive → gates → slices → review →
  security → connection_graph → analysis_plan → cross_unit_analysis →
  synthesis → followup_plan → report.md → report.html → run.json →
  artifact_manifest.json → brief.json/brief.md → 디스크 검증.
...
```json
{
  ...
  "tool": { "name": "git-scv", "version": "0.3.0" },
  "command": {
    "program": "git-scv",
    "subcommand": "inspect",
...
| 번호 | 검사 | 실패 문자열(그대로) |
| --- | --- | --- |
| V01 | 23개 inspect 산출물 파일이 모두 존재 (0801, 0901) | `V01: 산출물 파일 누락: {이름들}` |
