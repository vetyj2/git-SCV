...
3. Summarize `verdict`, `action_required`, `required_actions`,
   `reason_codes`, `artifact_manifest_sha256`, and `source_fingerprint_hash`
   to the user before any next action.
...
6. If Hermes produces unit-analysis files, run `git-scv validate-unit` or
